...
- `Woven Bundle`, a normal bundle with a custom name
...
Wool wear, woven saddle, and woven bundle recipes are added to the crafting
recipe book under the equipment category. The plugin unlocks its recipe keys for
players when they join, and for already-online players when the plugin is enabled
during a reload, so the recipes appear in recipe book search. Woven bundle
recipes are registered per wool color, and woven saddle recipes are registered
per wool color and slab type, so the client receives concrete recipe book
entries instead of broad ingredient-choice recipes.
